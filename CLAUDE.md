...
Phase 1 (Linux headless server) shipped and is validated end-to-end against the `forge` dev box (systemd unit running, CLI works against the live socket). Phase 2 (remote backend in the UI) is code complete and validated at the Rust + CLI layer end-to-end through a real SSH unix-socket forward against `forge`; the Tauri UI portion has been built but has not yet been manually exercised in a running app window. Phase 3 (auto SSH port forwarding) is feature complete: per-port toggle + sync + local-conflict probe + ControlMaster ownership + quit cleanup + startup auto-sync + 60s periodic safety-net timer (`forwards::start_auto_sync`) + per-backend "Excluded ports" sub-UI in Settings. The only Phase 3 piece deferred from the original plan is the server-pushed `StateChanged` event channel - we rely on the periodic 60s sync + post-mutation manual sync, which is acceptable for the dev-server use case. macOS notifications on local port conflict are also deferred (the failure surfaces in the indicator tooltip + toast). Phase 4 (polish) is not yet started.
